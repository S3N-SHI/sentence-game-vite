export class AudioManager {
  ...
  setupAudioElements() {
    ...
    this.killSound = document.getElementById('killSound') || new Audio('/sonido/kill.mp3');
    this.killSound.preload = 'auto';
    this.killSound.loop = true;
  ...
  /**
   * Reproduce música de tensión (kill.mp3)
   */
  reproducirMusicaTension() {
    if (this.musica && !this.musica.paused) {
      this.musica.pause();
    }
    if (this.killSound && this.killSound.paused) {
      this.killSound.play().catch(() => { });
    }
  }

  /**
   * Detiene música de tensión y vuelve a la normal
   */
  detenerMusicaTension() {
    if (this.killSound && !this.killSound.paused) {
